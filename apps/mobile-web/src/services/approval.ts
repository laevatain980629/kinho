...
export async function approveApproval(id: number): Promise<ApprovalItem> {
  return apiPost(`/approvals/${id}/approve`, {});
}

export async function rejectApproval(id: number, rejectReason: string): Promise<ApprovalItem> {
  return apiPost(`/approvals/${id}/reject`, { rejectReason });
}

export async function approveEscalation(id: number): Promise<unknown> {
